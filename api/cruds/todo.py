...
#post
async def add_todo(db: AsyncSession, add_schema: todo_schema.Todo)->todo_model.Todo:
	todo = todo_model.Todo(
				content=add_schema.content,
				deadline=add_schema.deadline,
				is_done=add_schema.is_done,
				created_at= datetime.datetime.now(config.JST)
				)
	db.add(todo)
	await db.commit()
...
async def update_todo(
	...
	if (not original.is_done) and todo_schema.is_done:    #終了したとき
		original.deleted_at = datetime.now(config.JST)
	...
	#original.created_atは変更しない
	original.updated_at = datetime.now(config.JST)
